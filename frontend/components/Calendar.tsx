...
export default function Calendar({ base_path, current_page_path, statusData = {} }: CalendarProps) {
    ...
    const getStatusClasses = (dateStr: string, isCurrentMonth: boolean): string => {
        ...
        switch (status) {
            ...
            case 'no_requests':
                // 誰も希望を提出していない
                return 'bg-white hover:bg-red-200 border-gray-200';
            default:
                // データがない場合（未来の日付など）
            ...
            <div className="p-2">
                ...
                <div className="grid grid-cols-7 mb-1">
                    {dayNames.map((dayName, idx) => (
                        <div
                            key={dayName}
                            className={`text-center text-xs font-bold py-2 ${idx === 0 ? 'text-red-500' : idx === 6 ? 'text-blue-500' : 'text-gray-500'}`}
                        >
                            {dayName}
                        </div>
                    ))}
                    ...
                    {calendarDays.map((day, idx) => {
                        ...
                        return (
                            <div
                                ...
                                className={`
                                    h-28 p-2 rounded-xl border-2 text-left transition-all duration-150 relative
                                    ${statusClasses} /* ★ここをステータス色に変更 */
                                    ${isClickable ? 'cursor-pointer' : 'cursor-default'}
                                    ${isToday ? 'border-red-400' : ''} /* 今日の強調 */
                                `}
                            >
                                {/* 日付の数字 (右上に配置) */}
