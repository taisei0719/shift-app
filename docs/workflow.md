...
- **コードレビュー対応・実装中に見つけた問題はSBIの範囲内でのみ直す**: CodeRabbit等のレビュー指摘や実装中に気づいたバグ・改善点は、着手中のSBIのタスク内容・DoDに含まれる範囲であればそのSBI内で直してよい。SBIの範囲外の内容（無関係なファイルの問題、別機能の改善など）はその場で実装せず、新しいSBI/PBI issueとして起票するに留める（スコープの肥大化・PRの巨大化を防ぐため）。
